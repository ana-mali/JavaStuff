...
public class A04Main {
	...
	private static void characterTable(final BST<Character> tree) {
        ...
        for (CountedValue<Character> z : tree.inOrder()) {
            totalCount += z.getCount();
        }


		System.out.print("Char    Count Percent\n");
		for (CountedValue<Character> x : tree.inOrder()) {
			double percent =(double) x.getCount()/totalCount*100;
			System.out.printf("%4s %,8d %7.2f\n", x.getValue(), x.getCount(),percent);
		}
	...
	private static void fillTree(final BST<Character> tree, final String string) {
		for (int i = 0; i < string.length(); i++) {
			...
			if (Character.isLetter(letter)) {
				...
				tree.insert(new CountedValue<Character>(letter));
			}
		}
		return;
	}
	...
	private static int retrieve(final BST<Character> tree, final Scanner fileScan) {
		...
		while (fileScan.hasNext()) {
			String phrase = fileScan.next();
			for (int i = 0; i < phrase.length(); i++) {
				Character letter = (char) phrase.charAt(i);
				if (Character.isLetter(letter)) {
					letter = Character.toUpperCase(letter);
					CountedValue<Character> value = tree.retrieve(new CountedValue<Character>(letter));
				}
			}
		}
		num=tree.getComparisons();
